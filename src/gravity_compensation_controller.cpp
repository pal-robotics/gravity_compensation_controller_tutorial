...
namespace force_control
{
                                         ...
                                         ros::NodeHandle& /*root_nh*/, ros::NodeHandle& control_nh)
  {
    ...
    for (size_t i = 0; i < joint_names_.size(); i++)
    {
      ...
      if (control_type == "actuated") // If joint is actuated
      {
        ...
        if (!control_nh.getParam("static_friction", friction_parameters.static_friction))
          ROS_WARN_STREAM("No static friction defined for joint "
                          << joint_names_[i] << ". Setting it to 0.0");
      ...
      else // If static joint
      {
        try
        {
          // Try to get a joint state handle which only allows us to read the current states of the joint
        ...
        {
          ROS_ERROR_STREAM("Could not find joint " << joint_names_[i]
  ...
  void GravityCompensationTutorial::update(const ros::Time& time, const ros::Duration& period)
  {
    ...
    for (size_t i = 0; i < joint_names_.size(); ++i)
    {
      ...
      if (joint_types_[joint_names_[i]] == JointType::ACTUATED)
      {
        // Translate the calculated torque to desired effort by integrating the frictions of the motor + the ger ration + motor constants
